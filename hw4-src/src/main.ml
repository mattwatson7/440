...
| Match (e1, t1, e2, s1, s2, e3) -> let list = (typeof env e1) in (match list with Tlist(t) -> (if t = t1 then let t2 = (typeof env e2) in let env2 = (insert (insert env s1 t1) s2 (Tlist t1)) in let t3 = (typeof env2 e3) in (if t2 = t3 then t2 else raise Nottyped) else raise Nottyped) | _ -> raise Nottyped)
